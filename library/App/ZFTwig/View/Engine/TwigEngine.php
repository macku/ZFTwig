 ...
 */
class App_ZFTwig_View_Engine_TwigEngine extends App_View_Engine_Abstract
{
    /**
    ...
    public function init(array $config = array())
    {
        ...
        $loader = new App_ZFTwig_Loader_FileLoader($this->getView()->getScriptPaths());
        $twigEnvironment = new App_ZFTwig_Environment($this->getView(), $loader, $options);
        ...
        if (array_key_exists('extensions', $config) && is_array($config['extensions'])) {
            foreach($config['extensions'] as $extension) {
                ...
                if (!class_exists($extensionClass)) {
                    throw new App_ZFTwig_View_Engine_TwigEngine_Exception('Extension class doesn\'t exist: ' . $extension['class']);
                }
        ...
        if (array_key_exists('globals', $config) && is_array($config['globals'])) {
            if (array_key_exists('class', $config['globals'])) {
                ...
                if (!array_key_exists('name', $config['globals']) || empty($config['globals']['name'])) {
                    throw new App_ZFTwig_View_Engine_TwigEngine_Exception('You must provide a name for globals (e.g. "app")');
                }
                $globalsName = $config['globals']['name'];
                
                if (!class_exists($globalsClass)) {
                    throw new App_ZFTwig_View_Engine_TwigEngine_Exception('Class for globals doesn\'t exist: ' . $globalsClass);
                }
            }
        }
        if ($globalsClass != '' && $globalsName != '') {
            ...
            $twigEnvironment->addGlobal('app', new App_ZFTwig_GlobalVariables());
        }
     ...
     * @return App_ZFTwig_View_Engine_TwigEngine
     */
